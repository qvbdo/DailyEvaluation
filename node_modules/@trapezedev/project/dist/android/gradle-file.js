...
class GradleFile extends vfs_1.VFSStorable {
    constructor(filename, vfs) {
        ...
        this.gradleCommitFn = async (file) => {
            await (0, fs_2.assertParentDirs)(file.getFilename());
    ...
    async replaceProperties(pathObject, toReplace, exact = false) {
        ...
        if (!found.length) {
            ...
            if (foundParent.length) {
                this.insertIntoGradleFile([toReplace], foundParent[0], definitions_1.AndroidGradleInjectType.Infer);
                return;
            }
            else {
    ...
    async replaceInGradleFile(toInject, targetNode) {
        ...
        let lines = [];
        this.createGradleSource([toInject], lines /* out */, detectedIndent.indent, undefined, targetNode.node, definitions_1.AndroidGradleInjectType.Infer);
     ...
     * if it can match on a sub-object
     **/
    async insertProperties(pathObject, toInject, type = definitions_1.AndroidGradleInjectType.Method, exact = false) {
        await this.parse();
        if (!this.parsed) {
            throw new Error('Call parse() first to load Gradle file');
        }
    ...
    async insertFragment(pathObject, toInject, exact = false) {
        ...
        const target = found[0];
        return this.insertIntoGradleFile(toInject, target, definitions_1.AndroidGradleInjectType.Infer);
    }
    /**
    ...
    async parse() {
        ...
        if (!this.tempFile) {
            ...
            this.tempFile = (0, path_1.join)((0, fs_1.mkdtempSync)((0, path_1.join)(os_1.default.tmpdir(), 'trapeze-')), 'temp.gradle');
            ...
            for (const key of keys) {
                ...
                if (Array.isArray(editEntry)) {
                    if (typeof editEntry[0] === 'object') {
                        ...
                        if (targetNode.type === 'variable' || type === definitions_1.AndroidGradleInjectType.Variable) {
